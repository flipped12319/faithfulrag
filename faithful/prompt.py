...
class PromptGenerator:
    ...
    def generate_qa_prompt_normal_cot_with_conflict(self, context, question, conflict_information,options=None, facts=None):
        normal_w_facts_prompt = """
               You are a reasoning assistant that must output strictly valid JSON.
               ...
               3. Refer to the **Context** and **Conflict information** If the facts do not contain enough information to answer the question, or if additional information is needed.
                  - If Conflict information indicates "has_conflict" = yes:
                    - Ignore or discount the corresponding conflicting context contents.
                    - Use the provided "internal_knowledge" field as the ground truth instead.
               4. Finally,output the answer.The answer must directly answers the question and do not conclude irrelevant information
                 ...
                 ## Important Rules:
                   ...
                   - Output **ONLY** a valid JSON object — no markdown, no text before or after, no `<think>` tags, no explanations.
                   - Use escaped quotes like \" inside strings.\
                   - The final answer should directly answers the question，do not judge the correctness of the internal_knowledge
        ...
        if options is None:
            if facts is None:
                ...
                return normal_w_facts_prompt.format(question=question, facts=facts, context=context,conflict_information=conflict_information)
        else:
            if facts is None:
                return choice_wo_facts_prompt.format(question=question, context=context, options=options)
            else:

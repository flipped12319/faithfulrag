...
for index,value in enumerate(newDataArr):
    ...
    for res_index,res_value in enumerate(res_json):
        ...
        res_value = re.sub(r'\n?\s*```\s*$', '', res_value)
        # res_value = re.sub(r'}\s*[\s\S]*$', '}', res_value)
        print(res_value)
...
for value in contextual_chunks_topk:
    promptArr=[]
    topk_chunks=value["topk_chunks"]
    print(topk_chunks)
    for chunk_obj in topk_chunks:
        chunk=chunk_obj["chunk"]
        prompt=prompt_generator.conflict_detection_layer1(chunk)
        promptArr.append(prompt)
    system_prompt="You are a lightweight factual consistency screener."
    res=llm.generate_all(system_prompt=system_prompt,prompt_list=promptArr)
    print(res)
    promptArr = []
    for index,res_value in enumerate(res):
        if res_value.strip()=='Yes':
            print("happened ",index)
            system_prompt="You are a factual verifier."
            print(topk_chunks[index]["chunk"])
            prompt = prompt_generator.information_extraction(topk_chunks[index]["chunk"])
            promptArr.append(prompt)
            print(promptArr)
    claims = llm.generate_all(system_prompt=system_prompt, prompt_list=promptArr)
    conflict_arr=[]
    for claim_arr in claims:
        print("claim_arr",claim_arr)
        real_claim_arr=json.loads(claim_arr)
        promptArr=[]
        for claim in real_claim_arr:
            prompt = prompt_generator.conflict_detection_layer2(claim)
            promptArr.append(prompt)
        system_prompt="You are a factual verifier."
        res_arr = llm.generate_all(system_prompt=system_prompt, prompt_list=promptArr)
        for res_json in res_arr:

            # res_json=re.sub(r'^\s*Output:\s*', '', res_json)
            # res_json = re.sub(r'^\s*output:\s*', '', res_json)
            res_json = re.sub(r"^```json\s*", "", res_json)

            res_json = re.sub(r'^[\s\S]*?(?=\{)', '', res_json)
            res_json = re.sub(r'\n?\s*```\s*$', '', res_json)

            print(res_json)
            res=json.loads(res_json)
            print(res)
            if res["has_conflict"]=='yes':
                conflict_arr.append(res)
    conflict_all_arr.append(conflict_arr)
print(conflict_all_arr)
for value in conflict_all_arr:
    print(value)
print("conflict_all_arr",len(conflict_all_arr))
print("dataArr",len(dataArr))
print("contextual_chunks_topk",len(contextual_chunks_topk))



predict_answer_normal_conflict=self_think_module.predict_answer_normal_cot_with_conflict(dataArr,contextual_chunks_topk,conflict_all_arr)
print(predict_answer_normal_conflict)
...
total_num_f=0
ex_acc=0
for answer,item in zip(predict_answer_normal_conflict.values(),dataArr):
        print("\nfaithful")
        print('item; ',item["id"])
